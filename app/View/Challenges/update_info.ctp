...
<div id="startbridge-information" class="box-startbridge box-white rounded">
	...
	<div class="box-content information-fields-1">
		<ul class="fieldset">
			...
			<li id="add_youtube">
				<p class="label"><?php echo __('Add YouTube Video Link') ?> <span class="action-preview" style="margin-left: 180px; font-size: 13px;"><a href="#" onclick="$('#HelpDialog').dialog('open');return false;"><?php echo __('Help?') ?></a></span></p>
				<input type="text" class="checkdefault" value="<?php echo (@$challenge['Attachment'][0]['type'] == 'C' ? $challenge['Attachment'][0]['file_location'] : '' ); ?>" name="video_embed" size="60" />
			</li>
			<li id="add_offline">
			...
			<ol class="fieldset">
				...
				if(@$challenge['Question']){
					foreach(@$challenge['Question'] as $k=>$question){ ?>
						<li>
							<p><!-- <input type="text" class="checkdefault assignment-section-title" value="<?php echo $question['section']; ?>" name="challenge[Question][<?php echo $k; ?>][section]" /> --></p>
...
<span id="meridianValidate" style="display:none;">
	<p class="textAlignCenter red" style="margin-left:315px; color:red"><?php echo __('You must select AM or PM') ?></p>
</span>
...
function validate_info(){
	...
	else if($('#responses_due_meridian').val() == '' || $('#answers_due_meridian').val() == '') $('#meridianValidate').show();
